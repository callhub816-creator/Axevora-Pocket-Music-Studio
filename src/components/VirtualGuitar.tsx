...
export default function VirtualGuitar({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-12 w-full max-w-4xl">
      <div className="w-full h-48 bg-amber-900/40 rounded-3xl border-8 border-amber-900/60 flex flex-col justify-around p-4 relative shadow-2xl overflow-hidden">
         {/* Strings */}
         {[1, 2, 3, 4, 5, 6].map((s) => (
         ...
         <div className="absolute inset-0 flex items-center justify-around z-20">
           {CHORDS.map((chord) => (
             <motion.button
               ...
               className="glass-panel px-6 py-4 border-2 border-white/20 hover:border-accent-secondary"
             >
